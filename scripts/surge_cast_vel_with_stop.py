...
class OdorTrackerNode:
    ...
    def tracking_step(self):
        ...
        if not self.reached_start_position:
            ...
            # If close to the start position, hover and wait for user input
            if dist_to_start < 3.0:
                # Only prompt for input once
                if not hasattr(self, 'input_prompt_shown'):
                    rospy.loginfo("Reached starting position. Hovering and waiting for user input.")
                    rospy.loginfo("Press Enter in the terminal to begin odor tracking.")
                ...
                # Keep hovering at the current position
                ...
                # Continue moving to the start position
        ...
        # Check if we should start tracking yet
        if not self.start_tracking:
            # Continue hovering while waiting for user input
            hover_pose = PositionTarget()
            hover_pose.coordinate_frame = PositionTarget.FRAME_LOCAL_NED
            hover_pose.type_mask = (
                PositionTarget.IGNORE_VX + PositionTarget.IGNORE_VY + PositionTarget.IGNORE_VZ +
